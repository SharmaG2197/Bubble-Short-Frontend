...
class Bubble {
  ...
  Bubble(this.letter, this.word, this.imageUrl, this.isSpecial,
...
class KidsGameScreen extends StatefulWidget {
  ...
  @override
  State<KidsGameScreen> createState() => _AdultsGameScreenState();
}

class _AdultsGameScreenState extends State<KidsGameScreen> {
  ...
  void _startGame() {
    ...
    _bubbleTimer = Timer.periodic(
      ...
      (_) {
        if (!_paused && !_showingImage) _fetchBubble();
      },
    );
  }
  ...
  Future<void> _fetchBubble() async {
    ...
    if (response.statusCode == 200) {
      ...
      final bubble = Bubble(
        ...
        'https://bubble-shot-api.onrender.com${data['image_url']}',
        isSpecial,
        leftPosition: left,
      );
  ...
  void _handleUserInput(String input) async {
    if (_paused || _showingImage) return;
    ...
    if (matchIndex != -1) {
      ...
      _bubbleSound();
      _checkSpeed();
      _showImage(current.imageUrl, current.letter, current.word);
      _fetchBubble();
    } else {
      if (!mounted) return;
      setState(() {
  ...
  void _showImage(String url, String letter, String word) {
    _showingImage = true;
    showDialog(
      context: context,
